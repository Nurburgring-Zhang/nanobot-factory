...
# -*- coding: utf-8 -*-
"""
AIGC鎵瑰鐞嗗伐鍏?v5.4 鏈湴API鏈嶅姟鍣?鏇夸唬Supabase Edge Functions锛屾彁渚涘畬鏁寸殑鏈湴鍖栧悗绔湇鍔?"""
...
# FastAPI搴旂敤鍒濆鍖?app = FastAPI(
    title="AIGC鏈湴API鏈嶅姟",
    description="鏇夸唬Supabase鐨勫畬鏁存湰鍦板寲鍚庣鏈嶅姟",
    version="5.4.0"
)
...
# 鍏ㄥ眬缁勪欢鍒濆鍖?model_registry = ModelRegistry()
...
async def start_generation(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"鍚姩鐢熸垚浠诲姟澶辫触: {str(e)}")

async def execute_generation_task(task_id: str, task: GenerationTask):
    """鎵ц鐢熸垚浠诲姟"""
    try:
        # 鏇存柊浠诲姟鐘舵€佷负澶勭悊涓?        active_generations[task_id].status = "processing"
        update_task_status(task_id, "processing", 0)
        
        # 鑾峰彇鍚庣绠＄悊鍣?        backend_manager = get_backend_manager()
        
        # 鏍规嵁浠诲姟绫诲瀷鎵ц涓嶅悓鐨勭敓鎴愰€昏緫
...
async def execute_image_generation(task: GenerationTask) -> List[str]:
    ...
    if not model_config:
        raise ValueError(f"妯″瀷鏈壘鍒? {task.model_name}")
    ...
    # 浣跨敤鐜绠＄悊鍣ㄦ墽琛岀敓鎴?    from main import EngineFactory
...
async def get_generation_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"鑾峰彇鐢熸垚鐘舵€佸け璐? {str(e)}")
...
async def model_management(
    ...
    try:
        ...
        if action == "list":
            ...
            # 鑾峰彇閲囨牱鍣ㄥ垪琛?            samplers = [
                {"id": "dpmpp_2m", "name": "DPM++ 2M", "description": "蹇€熺ǔ瀹?, "speed": "fast", "quality": "high", "memory_usage": "medium"},
                {"id": "dpmpp_2m_sde", "name": "DPM++ 2M SDE", "description": "楂樿川閲廠DE", "speed": "medium", "quality": "high", "memory_usage": "high"},
                {"id": "euler", "name": "Euler", "description": "绠€鍗曞揩閫?, "speed": "fast", "quality": "standard", "memory_usage": "low"},
                {"id": "euler_a", "name": "Euler Ancestral", "description": "鏇村垱鎰?, "speed": "fast", "quality": "standard", "memory_usage": "low"},
                {"id": "lcm", "name": "LCM", "description": "娼滃湪涓€鑷存€фā鍨?, "speed": "fast", "quality": "draft", "memory_usage": "low"}
            ]
            ...
            # 鑾峰彇璋冨害鍣ㄥ垪琛?            schedulers = [
                ...
                {"id": "karras", "name": "Karras", "description": "Karras鍣０璋冨害", "type": "karras", "stability": "high"},
...
async def download_file(file_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="鏂囦欢鏈壘鍒?)
...
async def startup_event():
    """搴旂敤鍚姩鏃剁殑鍒濆鍖?""
    print("馃殌 鍚姩AIGC鏈湴API鏈嶅姟鍣?..")
    
    # 鍒濆鍖栨暟鎹簱
    init_database()
    print("鉁?鏁版嵁搴撳垵濮嬪寲瀹屾垚")
    
    # 妫€鏌ョ幆澧?    environment_manager.print_environment_info()
    
    # 鍚姩浠诲姟璋冨害鍣?    print("鉁?浠诲姟璋冨害鍣ㄥ凡鍚姩")
...
async def init_annotation_system():
    ...
    try:
        ...
        if ANNOTATION_SYSTEM_AVAILABLE:
            ...
            print("鈿狅笍 鏁版嵁鏍囨敞绯荤粺妯″潡鏈壘鍒帮紝閮ㄥ垎鍔熻兘涓嶅彲鐢?)
    except Exception as e:
        print(f"鈿狅笍 鏍囨敞绯荤粺鍒濆鍖栧け璐? {e}")
...
async def get_hot_topics(category: str = None, limit: int = 20):
    """鑾峰彇鐑偣璇濋"""
    try:
        # 妯℃嫙鐑偣璇濋鏁版嵁 (瀹為檯搴斾粠ai_automation.py鑾峰彇)
        topics = [
            {'id': 1, 'topic': 'AI鐢熸垚瑙嗛鎶€鏈?, 'trend': 'up', 'heat': 12500, 'category': '鎶€鏈?, 'last_update': '鍒氬垰', 'source': '鎶€鏈ぞ鍖?, 'related_tags': ['AIGC', '瑙嗛鐢熸垚']},
            ...
            {'id': 4, 'topic': 'VRM 3D铏氭嫙褰㈣薄', 'trend': 'up', 'heat': 6500, 'category': '鎶€鏈?, 'last_update': '30鍒嗛挓鍓?, 'source': '鎶€鏈鍧?, 'related_tags': ['VRM', '3D']},
            {'id': 5, 'topic': 'ComfyUI 宸ヤ綔娴?, 'trend': 'up', 'heat': 5200, 'category': '宸ュ叿', 'last_update': '45鍒嗛挓鍓?, 'source': '宸ュ叿绀惧尯', 'related_tags': ['ComfyUI', '宸ヤ綔娴?]},
            {'id': 6, 'topic': 'AI Agent 鑷姩鍖?, 'trend': 'up', 'heat': 4800, 'category': '鎶€鏈?, 'last_update': '1灏忔椂鍓?, 'source': '鎶€鏈崥瀹?, 'related_tags': ['Agent', '鑷姩鍖?]},
            ...
            {'id': 9, 'topic': 'FLUX 妯″瀷浼樺寲', 'trend': 'up', 'heat': 3500, 'category': 'AI', 'last_update': '30鍒嗛挓鍓?, 'source': '鎶€鏈鍧?, 'related_tags': ['FLUX', '浼樺寲']},
...
async def refresh_hot_topics(categories: List[str] = None):
    """鍒锋柊鐑偣璇濋"""
    try:
        # 妯℃嫙鍒锋柊鐑偣璇濋 (瀹為檯搴旇皟鐢╝i_automation.py)
        import random
        topics = []
        for i in range(10):
            topics.append({
                ...
                'category': random.choice(['鎶€鏈?, 'AI', '鍒涗綔', '宸ュ叿', '绀惧尯']),
                'last_update': '鍒氬垰',
                'source': '瀹炴椂鍒锋柊'
            })
        return {
...
async def health_check():
    ...
    try:
        ...
        # 妫€鏌ユ湇鍔＄姸鎬?        services = {
            ...
            'gpu': True,  # 绠€鍖栨鏌?            'storage': disk.percent < 95
        }
...
async def shutdown_event():
    """搴旂敤鍏抽棴鏃剁殑娓呯悊"""
    print("馃洃 姝ｅ湪鍏抽棴AIGC鏈湴API鏈嶅姟鍣?..")
    
    # 鍋滄鎵€鏈夋椿璺冧换鍔?    for task_id, task in active_generations.items():
