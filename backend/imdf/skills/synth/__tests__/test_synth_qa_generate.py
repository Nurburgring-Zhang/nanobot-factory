"""Tests for synth skill: synth_qa_generate (QA 对生成)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_qa_generate import qa_generate, QaGenerateInput
...
async def test_happy_path():
    ...
    params = {'context': 'sample context'}
    out = await qa_generate(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_qa_generate"
...
async def test_with_pydantic_schema():
    ...
    schema = QaGenerateInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await qa_generate(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
