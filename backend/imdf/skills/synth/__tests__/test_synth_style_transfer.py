"""Tests for synth skill: synth_style_transfer (风格迁移)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_style_transfer import style_transfer, StyleTransferInput
...
async def test_with_pydantic_schema():
    ...
    schema = StyleTransferInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await style_transfer(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
