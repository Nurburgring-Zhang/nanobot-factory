"""Tests for synth skill: synth_back_translate (回译增强)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_back_translate import back_translate, BackTranslateInput
...
async def test_with_pydantic_schema():
    ...
    schema = BackTranslateInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await back_translate(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
