"""Tests for synth skill: synth_video_temporal (时序动作描述)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_video_temporal import video_temporal, VideoTemporalInput
...
async def test_happy_path():
    ...
    params = {'video_ref': 'vid://test/001'}
    out = await video_temporal(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_video_temporal"
...
async def test_with_pydantic_schema():
    ...
    schema = VideoTemporalInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await video_temporal(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
