"""Tests for synth skill: synth_paraphrase (文本改写)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_paraphrase import paraphrase, ParaphraseInput
...
async def test_happy_path():
    ...
    params = {'text': 'Sample text to paraphrase'}
    out = await paraphrase(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_paraphrase"
...
async def test_with_pydantic_schema():
    ...
    schema = ParaphraseInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await paraphrase(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
