"""Tests for synth skill: synth_dialog_generate (多轮对话生成)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_dialog_generate import dialog_generate, DialogGenerateInput
...
async def test_with_pydantic_schema():
    ...
    schema = DialogGenerateInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await dialog_generate(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
