"""Tests for synth skill: synth_image_caption (图像描述合成)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_image_caption import image_caption, ImageCaptionInput
...
async def test_happy_path():
    ...
    params = {'image_ref': 'img://test/001'}
    out = await image_caption(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_image_caption"
...
async def test_with_pydantic_schema():
    ...
    schema = ImageCaptionInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await image_caption(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
