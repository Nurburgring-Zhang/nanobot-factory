"""Tests for synth skill: synth_seed_expand (种子词扩展)."""
...
async def test_happy_path():
    ...
    params = {'seed_words': ['cat', 'dog']}
    out = await seed_expand(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_seed_expand"
...
async def test_with_pydantic_schema():
    ...
    schema = SeedExpandInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await seed_expand(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
