"""Tests for synth skill: synth_translate_en (英译中)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_translate_en import translate_en, TranslateEnInput
...
async def test_with_pydantic_schema():
    ...
    schema = TranslateEnInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await translate_en(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
