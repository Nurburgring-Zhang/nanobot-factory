"""Tests for synth skill: synth_3d_caption (3D 场景描述)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_3d_caption import three_d_caption, ThreeDCaptionInput
...
async def test_happy_path():
    ...
    params = {'scene_ref': 'scene://test/001'}
    out = await three_d_caption(_input(**params))
    assert isinstance(out, SkillOutput)
    assert out.success is True, f"unexpected failure: {out.error!r}"
    assert out.error == ""
    assert isinstance(out.result, dict)
    assert out.metadata.get("skill_module") == "synth_3d_caption"
...
async def test_with_pydantic_schema():
    ...
    schema = ThreeDCaptionInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await three_d_caption(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
