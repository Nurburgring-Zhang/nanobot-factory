"""Tests for synth skill: synth_video_caption (视频描述合成)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_video_caption import video_caption, VideoCaptionInput
...
async def test_with_pydantic_schema():
    ...
    schema = VideoCaptionInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await video_caption(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
