"""Tests for synth skill: synth_translate_zh (中译英)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_translate_zh import translate_zh, TranslateZhInput
...
async def test_with_pydantic_schema():
    ...
    schema = TranslateZhInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await translate_zh(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
