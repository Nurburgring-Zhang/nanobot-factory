"""Tests for synth skill: synth_caption_expand (短描述扩写为长描述)."""
from __future__ import annotations

import pytest

from backend.skills import SkillInput, SkillOutput
from imdf.skills.synth.synth_caption_expand import caption_expand, CaptionExpandInput
...
async def test_with_pydantic_schema():
    ...
    schema = CaptionExpandInput.model_json_schema()
...
async def test_invalid_params_returns_error():
    ...
    out = await caption_expand(SkillInput(prompt="x", params={"bogus_field": "bad"}, context={}))
    assert isinstance(out, SkillOutput)
