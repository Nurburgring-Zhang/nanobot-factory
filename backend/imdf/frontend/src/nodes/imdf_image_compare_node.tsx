 ...
 * 类型 key : image-compare
 * 组件     : ImageCompareNode
 * Data in  : NodeDataShape<'image-compare'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('image-compare', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'image-compare'
...
const ImageCompareNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('image-compare', (p.data as Partial<NodeDataShape<'image-compare'>>) || undefined);
