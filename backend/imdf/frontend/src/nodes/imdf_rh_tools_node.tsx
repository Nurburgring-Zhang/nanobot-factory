 ...
 * 类型 key : rh-tools
 * 组件     : RhToolsNode
 * Data in  : NodeDataShape<'rh-tools'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('rh-tools', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'rh-tools'
...
const RHToolsNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('rh-tools', (data as Partial<NodeDataShape<'rh-tools'>>) || undefined);
  // 启动器
