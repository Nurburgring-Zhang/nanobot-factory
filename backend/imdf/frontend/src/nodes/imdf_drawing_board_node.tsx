 ...
 * 类型 key : drawing-board
 * 组件     : DrawingBoardNode
 * Data in  : NodeDataShape<'drawing-board'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('drawing-board', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'drawing-board'
...
const DrawingBoardNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('drawing-board', (data as Partial<NodeDataShape<'drawing-board'>>) || undefined);
