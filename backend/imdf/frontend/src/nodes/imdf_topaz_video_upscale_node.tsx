 ...
 * 类型 key : topaz-video-upscale
 * 组件     : TopazVideoUpscaleNode
 * Data in  : NodeDataShape<'topaz-video-upscale'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('topaz-video-upscale', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'topaz-video-upscale'
...
function TopazVideoUpscaleNode({ id, data, selected }: { id: string; data: any; selected?: boolean }) {
  ...
  const d = mergeDefaultData('topaz-video-upscale', (data as Partial<NodeDataShape<'topaz-video-upscale'>>) || undefined);
