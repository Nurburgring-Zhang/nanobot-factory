 ...
 * 类型 key : fal-toolbox
 * 组件     : FalToolboxNode
 * Data in  : NodeDataShape<'fal-toolbox'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('fal-toolbox', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'fal-toolbox'
...
const FalToolboxNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('fal-toolbox', (data as Partial<NodeDataShape<'fal-toolbox'>>) || undefined);
