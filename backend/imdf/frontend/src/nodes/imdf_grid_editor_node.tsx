 ...
 * 类型 key : grid-editor
 * 组件     : GridEditorNode
 * Data in  : NodeDataShape<'grid-editor'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('grid-editor', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'grid-editor'
...
const GridEditorNode = ({ id, data, selected }: NodeProps) => {
  const d = mergeDefaultData('grid-editor', (data as Partial<NodeDataShape<'grid-editor'>>) || undefined);
