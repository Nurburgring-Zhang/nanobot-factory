 ...
 * 类型 key : material-set
 * 组件     : MaterialSetNode
 * Data in  : NodeDataShape<'material-set'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('material-set', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'material-set'
...
const MaterialSetNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('material-set', (data as Partial<NodeDataShape<'material-set'>>) || undefined);
