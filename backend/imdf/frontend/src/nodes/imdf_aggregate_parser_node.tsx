 ...
 * 类型 key : aggregate-parser
 * 组件     : AggregateParserNode
 * Data in  : NodeDataShape<'aggregate-parser'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('aggregate-parser', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'aggregate-parser'
...
const AggregateParserNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('aggregate-parser', (p.data as Partial<NodeDataShape<'aggregate-parser'>>) || undefined);
