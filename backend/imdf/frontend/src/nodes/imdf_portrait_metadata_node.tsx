 ...
 * 类型 key : portrait-metadata
 * 组件     : PortraitMetadataNode
 * Data in  : NodeDataShape<'portrait-metadata'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('portrait-metadata', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'portrait-metadata'
...
const PortraitMetadataNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('portrait-metadata', (p.data as Partial<NodeDataShape<'portrait-metadata'>>) || undefined);
