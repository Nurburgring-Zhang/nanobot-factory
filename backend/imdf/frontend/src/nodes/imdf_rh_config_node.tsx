 ...
 * 类型 key : rh-config
 * 组件     : RhConfigNode
 * Data in  : NodeDataShape<'rh-config'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('rh-config', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'rh-config'
...
const RhConfigNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('rh-config', (data as Partial<NodeDataShape<'rh-config'>>) || undefined);
