 ...
 * 类型 key : video
 * 组件     : VideoNode
 * Data in  : NodeDataShape<'video'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('video', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'video'
...
const VideoNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('video', (data as Partial<NodeDataShape<'video'>>) || undefined);
