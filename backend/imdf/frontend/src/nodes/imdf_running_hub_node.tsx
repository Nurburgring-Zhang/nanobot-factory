 ...
 * 类型 key : running-hub
 * 组件     : RunningHubNode
 * Data in  : NodeDataShape<'running-hub'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('running-hub', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'running-hub'
...
const RunningHubNode = ({ id, data, selected, type }: NodeProps) => {
  ...
  const d = mergeDefaultData('running-hub', (data as Partial<NodeDataShape<'running-hub'>>) || undefined);
