 ...
 * 类型 key : frame-pair
 * 组件     : FramePairNode
 * Data in  : NodeDataShape<'frame-pair'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('frame-pair', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'frame-pair'
...
const FramePairNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('frame-pair', (p.data as Partial<NodeDataShape<'frame-pair'>>) || undefined);
