 ...
 * 类型 key : storyboard-grid
 * 组件     : StoryboardGridNode
 * Data in  : NodeDataShape<'storyboard-grid'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('storyboard-grid', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'storyboard-grid'
...
const StoryboardGridNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('storyboard-grid', (p.data as Partial<NodeDataShape<'storyboard-grid'>>) || undefined);
