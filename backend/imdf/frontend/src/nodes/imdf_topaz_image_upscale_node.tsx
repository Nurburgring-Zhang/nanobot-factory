 ...
 * 类型 key : topaz-image-upscale
 * 组件     : TopazImageUpscaleNode
 * Data in  : NodeDataShape<'topaz-image-upscale'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('topaz-image-upscale', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'topaz-image-upscale'
...
function TopazImageUpscaleNode({ id, data, selected }: { id: string; data: any; selected?: boolean }) {
  ...
  const d = mergeDefaultData('topaz-image-upscale', (data as Partial<NodeDataShape<'topaz-image-upscale'>>) || undefined);
