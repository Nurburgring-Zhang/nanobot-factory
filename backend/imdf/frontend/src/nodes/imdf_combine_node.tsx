 ...
 * 类型 key : combine
 * 组件     : CombineNode
 * Data in  : NodeDataShape<'combine'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('combine', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'combine'
...
const CombineNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('combine', (p.data as Partial<NodeDataShape<'combine'>>) || undefined);
