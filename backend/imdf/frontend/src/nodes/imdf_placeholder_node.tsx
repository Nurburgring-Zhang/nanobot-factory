 ...
 * 类型 key : placeholder
 * 组件     : PlaceholderNode
 * Data in  : NodeDataShape<'placeholder'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('placeholder', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'placeholder'
...
// Node color table (avoids Tailwind dynamic class issues)
 ...
 * Phase 1: display node type + metadata only (preparing for Phase 2/3 business logic)
 */
const PlaceholderNode = ({ data, type, selected }: NodeProps) => {
  const meta = getNodeMeta(type as string);
  const d = mergeDefaultData('placeholder', (data as Partial<NodeDataShape<'placeholder'>>) || undefined);
  const label = d?.label || meta?.label || type;
