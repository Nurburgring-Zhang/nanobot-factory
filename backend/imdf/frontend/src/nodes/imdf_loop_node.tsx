 ...
 * 类型 key : loop
 * 组件     : LoopNode
 * Data in  : NodeDataShape<'loop'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('loop', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'loop'
...
const LoopNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('loop', (p.data as Partial<NodeDataShape<'loop'>>) || undefined);
  ...
  const runSerial = useCallback(async (): Promise<void> => {
    ...
    //          Recompute downstream type='output' nodes from current nodes/edges via BFS on execSubIds (including 2nd-level links).
