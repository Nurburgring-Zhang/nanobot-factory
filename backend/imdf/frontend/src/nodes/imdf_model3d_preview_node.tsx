 ...
 * 类型 key : model3d-preview
 * 组件     : Model3dPreviewNode
 * Data in  : NodeDataShape<'model3d-preview'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('model3d-preview', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'model3d-preview'
...
const Model3DPreviewNode = ({ id, data, selected }: NodeProps) => {
  ...
  // model3d-preview 暂未在 types.NodeDataMap / DEFAULTS 中定义 (待补);
  // 这里 cast 类型 key 以满足 mergeDefaultData 签名.
  const d = mergeDefaultData('model3d-preview' as never, (data as Partial<NodeDataShape<'model3d-preview'>>) || undefined) as any;
