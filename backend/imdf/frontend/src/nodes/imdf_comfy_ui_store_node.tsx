 ...
 * 类型 key : comfy-ui-store
 * 组件     : ComfyUiStoreNode
 * Data in  : NodeDataShape<'comfy-ui-store'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('comfy-ui-store', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'comfy-ui-store'
...
const ComfyUIStoreNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('comfy-ui-store', (data as Partial<NodeDataShape<'comfy-ui-store'>>) || undefined);
