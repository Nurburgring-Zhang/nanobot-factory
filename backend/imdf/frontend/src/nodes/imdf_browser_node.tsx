 ...
 * 类型 key : browser
 * 组件     : BrowserNode
 * Data in  : NodeDataShape<'browser'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('browser', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'browser'
...
const BrowserNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('browser', (p.data as Partial<NodeDataShape<'browser'>>) || undefined);
