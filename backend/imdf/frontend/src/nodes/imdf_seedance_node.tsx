 ...
 * 类型 key : seedance
 * 组件     : SeedanceNode
 * Data in  : NodeDataShape<'seedance'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('seedance', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'seedance'
...
const SeedanceNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('seedance', (data as Partial<NodeDataShape<'seedance'>>) || undefined);
