 ...
 * 类型 key : relay
 * 组件     : RelayNode
 * Data in  : NodeDataShape<'relay'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('relay', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'relay'
...
const RelayNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('relay', (p.data as Partial<NodeDataShape<'relay'>>) || undefined);
