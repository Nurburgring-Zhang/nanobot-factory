 ...
 * 类型 key : image
 * 组件     : ImageNode
 * Data in  : NodeDataShape<'image'> (用户/上游配置 + 节点自身状态)
...
const ImageNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('image', (data as Partial<NodeDataShape<'image'>>) || undefined);
  ...
  // Note: outputs merged, sorted list only; no longer re-collects from edges/nodes in-place.
