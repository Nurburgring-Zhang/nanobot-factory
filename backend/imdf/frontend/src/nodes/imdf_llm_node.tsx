 ...
 * 类型 key : llm
 * 组件     : LlmNode
 * Data in  : NodeDataShape<'llm'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('llm', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'llm'
...
const LLMNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('llm', (data as Partial<NodeDataShape<'llm'>>) || undefined);
