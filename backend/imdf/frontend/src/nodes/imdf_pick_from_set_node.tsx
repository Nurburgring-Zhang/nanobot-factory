 ...
 * 类型 key : pick-from-set
 * 组件     : PickFromSetNode
 * Data in  : NodeDataShape<'pick-from-set'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('pick-from-set', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'pick-from-set'
...
const PickFromSetNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('pick-from-set', (p.data as Partial<NodeDataShape<'pick-from-set'>>) || undefined);
