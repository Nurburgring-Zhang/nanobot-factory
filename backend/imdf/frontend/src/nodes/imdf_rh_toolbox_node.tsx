 ...
 * 类型 key : rh-toolbox
 * 组件     : RhToolboxNode
 * Data in  : NodeDataShape<'rh-toolbox'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('rh-toolbox', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'rh-toolbox'
...
const RHToolboxNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('rh-toolbox', (data as Partial<NodeDataShape<'rh-toolbox'>>) || undefined);
