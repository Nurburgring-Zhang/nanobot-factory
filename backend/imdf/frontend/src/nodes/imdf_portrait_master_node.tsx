 ...
 * 类型 key : portrait-master
 * 组件     : PortraitMasterNode
 * Data in  : NodeDataShape<'portrait-master'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('portrait-master', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'portrait-master'
...
const PortraitMasterNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('portrait-master', (data as Partial<NodeDataShape<'portrait-master'>>) || undefined);
