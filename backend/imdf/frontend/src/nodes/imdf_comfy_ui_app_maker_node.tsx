 ...
 * 类型 key : comfy-ui-app-maker
 * 组件     : ComfyUiAppMakerNode
 * Data in  : NodeDataShape<'comfy-ui-app-maker'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('comfy-ui-app-maker', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'comfy-ui-app-maker'
...
const ComfyUIAppMakerNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('comfy-ui-app-maker', (data as Partial<NodeDataShape<'comfy-ui-app-maker'>>) || undefined);
