 ...
 * 类型 key : bp
 * 组件     : BpNode
 * Data in  : NodeDataShape<'bp'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('bp', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'bp'
...
const BpNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('bp', (p.data as Partial<NodeDataShape<'bp'>>) || undefined);
