 ...
 * 类型 key : preset-image
 * 组件     : PresetImageNode
 * Data in  : NodeDataShape<'preset-image'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('preset-image', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'preset-image'
...
const PresetImageNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('preset-image', (p.data as Partial<NodeDataShape<'preset-image'>>) || undefined);
