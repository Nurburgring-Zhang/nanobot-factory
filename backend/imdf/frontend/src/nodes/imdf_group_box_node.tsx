 ...
 * 类型 key : group-box
 * 组件     : GroupBoxNode
 * Data in  : NodeDataShape<'group-box'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('group-box', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'group-box'
...
const GroupBoxNode = ({ id, data, selected }: NodeProps) => {
  const d = mergeDefaultData('group-box', (data as Partial<NodeDataShape<'group-box'>>) || undefined) as unknown as GroupBoxData;
