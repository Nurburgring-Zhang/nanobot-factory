 ...
 * 类型 key : frame-extractor
 * 组件     : FrameExtractorNode
 * Data in  : NodeDataShape<'frame-extractor'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('frame-extractor', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'frame-extractor'
...
const FrameExtractorNode = (p: NodeProps) => {
  ...
  const d = mergeDefaultData('frame-extractor', (p.data as Partial<NodeDataShape<'frame-extractor'>>) || undefined);
