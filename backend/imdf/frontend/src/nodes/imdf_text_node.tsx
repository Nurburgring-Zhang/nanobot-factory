 ...
 * 类型 key : text
 * 组件     : TextNode
 * Data in  : NodeDataShape<'text'> (用户/上游配置 + 节点自身状态)
 * Data out : imageUrl/videoUrl/audioUrl/prompt/outputText 写回 data
 * 默认值   : mergeDefaultData('text', p.data) → defaults.ts
 * IO 文档   : NODE_IO_CONTRACTS → 类型 'text'
...
const TextNode = ({ id, data, selected }: NodeProps) => {
  ...
  const d = mergeDefaultData('text', (data as Partial<NodeDataShape<'text'>>) || undefined);
