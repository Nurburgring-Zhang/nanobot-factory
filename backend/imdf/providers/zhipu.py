"""P19-A2: ZhipuAI (智谱) provider adapter (OpenAI-compatible).

Provider family: zhipu
Protocol: OpenAI compatible (``/api/paas/v4/chat/completions``)
Auth: Bearer JWT (智谱生成 API key 即可)
Default models:
  - glm-4-plus      (文本旗舰)
  - glm-4v-plus     (多模态, image+text)
...
PRICE_INPUT_USD_PER_1K = 0.0004
PRICE_OUTPUT_USD_PER_1K = 0.0012
...
@dataclass
class ZhipuProvider:
    """In-memory descriptor for a ZhipuAI registration."""
    id: str = "zhipu"
    name: str = "智谱 GLM"
    family: str = "zhipu"
...
def _resolve_api_key(api_key: Optional[str] = None) -> str:
    ...
    env = os.environ.get("ZHIPUAI_API_KEY") or os.environ.get("ZHIPU_API_KEY") or ""
    return env.strip()
...
async def call_zhipu(
    ...
    """ZhipuAI call — OpenAI-compatible REST via shared helper."""
    if not provider:
    ...
    if not api_key:
        return {"ok": False, "code": "missing_api_key", "error": "Zhipu apiKey not configured"}
    ...
    if kind in ("image", "video"):
        return {"ok": False, "code": "unsupported_kind",
                "error": f"Zhipu adapter 不支持类型: {kind} (chat/vision only)"}
    ...
    try:
        ...
        try:
            ...
            return {"ok": True, "data": resp.json(),
                    "provider_id": provider.get("id") or "zhipu"}
        except Exception as e:
...
__all__ = [
    ...
    "ZhipuProvider", "call_zhipu", "compute_cost_usd",
]