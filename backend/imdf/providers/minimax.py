"""P19-B2: MiniMax (MiniMax abab) provider adapter (OpenAI-compatible).

Provider family: minimax
Protocol: OpenAI compatible (``/v1/chat/completions``)
Auth: Bearer API key
Default models:
  - abab-6.5s      (flagship 2026-Q1, 200k context)
  - abab-6.5-chat  (chat-tuned)
...
Note on naming: the underlying company is the Chinese LLM provider
"MiniMax" (formerly known as MiniMax). The module name follows the task
spec exactly (``minimax``); the public class and id are also
``minimax`` to keep registry IDs and env vars consistent.
"""
...
PRICE_INPUT_USD_PER_1K = 0.0006
PRICE_OUTPUT_USD_PER_1K = 0.0018
...
DEFAULT_VISION_MODELS: List[str] = [
    "abab-6.5s",  # abab-6.5s supports vision
]


@dataclass
class MiniMaxProvider:
    """In-memory descriptor for a MiniMax (MiniMax) registration."""
    id: str = "minimax"
    name: str = "MiniMax (MiniMax abab)"
    family: str = "minimax"
...
def _resolve_api_key(api_key: Optional[str] = None) -> str:
    ...
    env = os.environ.get("MINIMAX_API_KEY") or os.environ.get("ABAB_API_KEY") or ""
    return env.strip()
...
async def call_minimax(
    ...
    """MiniMax call — OpenAI-compatible REST via shared helper."""
    if not provider:
    ...
    if not api_key:
        return {"ok": False, "code": "missing_api_key", "error": "MiniMax apiKey not configured"}
    ...
    if kind in ("image", "video"):
        return {"ok": False, "code": "unsupported_kind",
                "error": "MiniMax adapter 不支持类型: chat/vision only"}
    ...
    try:
        ...
        try:
            ...
            return {"ok": True, "data": resp.json(),
                    "provider_id": provider.get("id") or "minimax"}
        except Exception as e:
...
__all__ = [
    ...
    "MiniMaxProvider", "call_minimax", "compute_cost_usd",
]
