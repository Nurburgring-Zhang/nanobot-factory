"""P19-A2: Tencent Hunyuan (混元) provider adapter (OpenAI-compatible).

Provider family: tencent
Protocol: OpenAI compatible (``/v1/chat/completions``)
Auth: Bearer API key (SecretId/SecretKey pair signed, but endpoint accepts bearer)
Default models:
  - hunyuan-pro       (旗舰)
  - hunyuan-standard  (性价比)
...
DEFAULT_API_BASE = "https://hunyuan.tencent.com/v1"
...
class TencentProvider:
    """In-memory descriptor for a Tencent Hunyuan registration."""
    id: str = "tencent"
    name: str = "腾讯混元 Hunyuan"
    family: str = "tencent"
...
def _resolve_api_key(api_key: Optional[str] = None) -> str:
    ...
    env = os.environ.get("HUNYUAN_API_KEY") or os.environ.get("TENCENT_API_KEY") or ""
    return env.strip()
...
async def call_tencent(
    ...
    """Tencent Hunyuan call — OpenAI-compatible REST via shared helper."""
    if not provider:
    ...
    if not api_key:
        return {"ok": False, "code": "missing_api_key", "error": "Tencent apiKey not configured"}
    ...
    if kind in ("image", "video"):
        return {"ok": False, "code": "unsupported_kind",
                "error": f"Tencent adapter 不支持类型: {kind} (chat/vision only)"}
    ...
    try:
        ...
        try:
            ...
            return {"ok": True, "data": resp.json(),
                    "provider_id": provider.get("id") or "tencent"}
        except Exception as e:
...
__all__ = [
    ...
    "TencentProvider", "call_tencent", "compute_cost_usd",
]