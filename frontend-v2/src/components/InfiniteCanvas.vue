...
<template>
  <div ref="containerRef" class="infinite-canvas" :class="{ 'is-connecting': !!pendingSourceId }" tabindex="0" @keydown="onKeyDown" @wheel.prevent="onWheel" @mousedown="onCanvasMouseDown" @mousemove="onMouseMove" @mouseup="onMouseUp" @mouseleave="onMouseUp">
    ...
    <svg class="ic-svg" :viewBox="`${viewBox.x} ${viewBox.y} ${viewBox.w} ${viewBox.h}`" preserveAspectRatio="xMidYMid meet" role="application" data-testid="ic-svg">
      <defs>
...
function addNode(type: CanvasNodeType, label: string, _color: string): CanvasNode {
